...
*/
    ...
    @Test
    public void test123FileBytePassword() throws ParseException {
        ...
        if (tests.size() > 0) {
            // copy the password of the first entry to the clipboard
            byte[] pass = tests.get(0).getPasswordAsBytes();
/*
            StringSelection selection = new StringSelection(pass);
            Toolkit.getDefaultToolkit().getSystemClipboard().setContents(selection, selection);
            printStream.println(pass + " copied to clip board");
*/
            // all the relevant entries should have the password 123
            byte[] pass2 = tests.get(0).getPasswordAsBytes();
            Assert.assertEquals(true, Arrays.equals(pass, pass2));
            Assert.assertEquals("123", new String(pass2));
        }

        List<? extends E> passwords = database.findEntries("password");
        Assert.assertEquals(4, passwords.size());
        for (Entry passwordEntry : passwords) {
            Assert.assertEquals(true, Arrays.equals(passwordEntry.getTitle().getBytes(), passwordEntry.getPasswordAsBytes()));
            printStream.println(passwordEntry.getTitle());
        }
        ...
        Assert.assertEquals(1, entries.size());
        assertEquals("pass", new String(entries.get(0).getPasswordAsBytes()));
